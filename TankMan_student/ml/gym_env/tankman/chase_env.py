...
class ResupplyEnv(TankManBaseEnv):
    ...
    def cal_forward_reward(self, obs: dict, action: int) -> float:
        ...
        elif obs[0] == (obs[1] + 4) % 8 and action == 2:    # BACKWARD_CMD (背對然後往後衝)
            forward_reward = 4.0
        elif obs[0] == (obs[1] + 4) % 8 and action == 1:    # FORWARD_CMD  (背對然後往前衝)
            forward_reward = -3.0
        elif (obs[0] == obs[1] or obs[0] == (obs[1] + 4) % 8) and (action != 1 or action != 2): #(無論是有對到敵人還是背對著敵人)
            forward_reward = -3.0
